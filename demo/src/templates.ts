...
export function templateProposal(s:Snapshot,id:string):Proposal{
 ...
 return {baseRevision:s.revision,field:'appearance',before:JSON.stringify(s.appearance),after:JSON.stringify(template.appearance),impact:`应用「${template.name}」的配色与线条样式。文字、节点位置、连接关系及工期保持不变。`};
}
