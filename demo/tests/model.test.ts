...
test('workspace shows three steps and template cards instead of review sidebar',async()=>{
 ...
 const html=renderToStaticMarkup(createElement(App));assert.match(html,/样式与模板/);assert.match(html,/商务蓝/);assert.match(html,/极简线框/);assert.match(html,/下载草稿 PNG/);assert.doesNotMatch(html,/检查图表|核对规则|class="review-panel|class="review-tabs/);assert.equal((html.match(/class="step-number"/g)||[]).length,3);
