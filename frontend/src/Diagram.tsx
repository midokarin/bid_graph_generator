...
export function Diagram({snapshot:s,svgRef,onPropose,locked=false}:{snapshot:Snapshot;svgRef?:React.Ref<SVGSVGElement>;onPropose?:(p:Proposal)=>void;locked?:boolean}){
 const {spec,style}=s.version; const a=s.appearance, font=s.fontSize*1.4, arrow=useId();
 ...
 function input(x:number,y:number,width:number){return editing&&<foreignObject data-editor="true" x={x} y={y} width={width} height={44}><input aria-label="编辑图中文字" autoFocus value={editing.text} maxLength={editing.task?TEXT_LIMITS.task:TEXT_LIMITS.flow} onChange={e=>setEditing({...editing,text:e.target.value})} onBlur={()=>finish()} onKeyDown={e=>{e.stopPropagation();if(e.key==='Enter')finish();if(e.key==='Escape')finish(true)}} style={{width:'100%',fontSize:18,padding:6,border:'2px solid #397fbd',background:'white',color:'black'}}/></foreignObject>}
 ...
 function down(e:React.PointerEvent<SVGGElement>,node:NodeGeometry){if(!onPropose||editing||e.button!==0)return;const p=point(e);dragRef.current={node,x:p.x,y:p.y};setSelected(node.id);e.currentTarget.setPointerCapture(e.pointerId)}
 ...
 return <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${layout.width} ${layout.height}`} role="img" aria-label={s.title} fill={a.textColor} style={{fontFamily:a.fontFamily,background,color:a.textColor,width:'100%',height:'auto'}}>
 ...
 {layout.nodes.map(node=>{const n=spec.nodes.find(n=>n.id===node.id)!;const {x,y,width:w,height:h}=node;const lines=wrapText(n.text);return <g key={n.id} data-node-id={n.id} data-node-type={n.type} role={onPropose?'button':undefined} tabIndex={onPropose?0:undefined} aria-label={`${n.text}，双击编辑，拖动调整位置`} onDoubleClick={()=>startEdit(n.id,n.text)} onKeyDown={e=>{if(e.key==='Enter')startEdit(n.id,n.text)}} onPointerDown={e=>down(e,node)} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{dragRef.current=null;setDrag(null)}} style={{cursor:onPropose?'move':'default',touchAction:'none'}}>
 ...
 <text x={x+w/2} y={y+h/2+font*.33-(lines.length-1)*font*.55} textAnchor="middle" fontSize={font} style={{userSelect:'none',pointerEvents:'none'}}>{lines.map((text,i)=><tspan key={i} x={x+w/2} dy={i?font*1.1:0}>{text}</tspan>)}</text>
 ...
 <text x="35" y={y+11} fontSize={font} role={onPropose?'button':undefined} tabIndex={onPropose?0:undefined} aria-label={`编辑任务：${task.text}`} onDoubleClick={()=>startEdit(task.id,task.text,true)} onKeyDown={e=>{if(e.key==='Enter')startEdit(task.id,task.text,true)}} style={{cursor:onPropose&&!locked?'text':'default'}}>{wrapText(task.text).map((text,i)=><tspan key={i} x={35} dy={i?font*1.1:-(wrapText(task.text).length-1)*font*.5}>{text}</tspan>)}</text>
