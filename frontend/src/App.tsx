...
const TaskWorkbench=memo(function TaskWorkbench({task,active,navigation,host,onReport}:{task:WorkspaceTask;active:boolean;navigation:ReactNode;host:HostAdapter;onReport:(id:string,summary:TaskSummary)=>void}){
 ...
 const [kind,setKind]=useState<Kind>('flowchart'),[text,setText]=useState(task.number===1?SAMPLES.flowchart.text:''),[title,setTitle]=useState(task.number===1?SAMPLES.flowchart.title:''),[bidMode,setBidMode]=useState('unknown'),[direction,setDirection]=useState<'DOWN'|'RIGHT'>('DOWN');
 ...
 function sample(k:Kind){if(busy||guardDraft())return;
  ...
  activate(k);setKind(k);if(TEMPLATES.find(t=>t.id===templateId)?.kind&&TEMPLATES.find(t=>t.id===templateId)?.kind!==k)setTemplateId('classic');const prior=fileContexts.current[k];setText(prior?.source??(task.number===1?SAMPLES[k].text:''));setTitle(prior?.title??(task.number===1?SAMPLES[k].title:''));const options=prior?.options??DEFAULT_EXPORT;setExportOptions(options);setInputDirty(false);
 }
 ...
 return <div className="app-shell task-workbench" hidden={!active} data-task-id={task.id}>
  ...
  <main className="editor-grid"><section ref={stylePanelRef} className="input-panel panel"><div className="panel-tabs"><button className={panel==='input'?'selected':''} onClick={()=>setPanel('input')}><FileText size={16}/>内容输入</button><button className={panel==='style'?'selected':''} onClick={()=>setPanel('style')}><Settings2 size={16}/>版式设置</button></div>
   ...
   <label className="field-label" htmlFor={`${task.id}-title`}>图题</label><input id={`${task.id}-title`} disabled={busy} value={title} onChange={e=>{setTitle(e.target.value);changeInput(true)}} placeholder="为这张图表命名"/>
