...
import {parseProject,type ProjectHandle} from './project/files';
...
export function App({host=standaloneAdapter}:{host?:HostAdapter}){
 const {versions,activeRevision,dirty,dirtyKinds,touch,markSaved,load,append,select,activate}=useWorkspace();const history=useMemo(()=>historyViews(versions),[versions]);const current=history.find(v=>v.revision===activeRevision);
 ...
 const [toast,setToast]=useState(''),[busy,setBusy]=useState(false),[proposal,setProposal]=useState<Proposal|null>(null),[modal,setModal]=useState<'export'|'help'|'replace'|'history'|'settings'|'files'|'open-confirm'|null>(null),[zoom,setZoom]=useState(100),[inputDirty,setInputDirty]=useState(false),[locked,setLocked]=useState(true);
 ...
 const pending=!!draft;
 const guardDraft=()=>{if(pending){notify('请先确认或取消画布中的修改。');return true}return false};
 ...
 useEffect(()=>{cancelEdits();setProposal(null)},[current?.version]);
 ...
 latestProject.current=current?{project_file_version:'1.0',diagram_type:kind,source_text:text,current_revision:activeRevision!,versions:versions as ProjectFile['versions'],metadata:{...metadata,export_options:{...exportOptions,paper:page,orientation},draft_title:title,bid_mode:bidMode,draft_direction:direction}}:null;
 ...
 const preview=draft??current??withAppearance(view(example),TEMPLATES.find(t=>t.id===templateId)!.appearance);
 ...
 function sample(k:Kind){if(busy||guardDraft())return;
  fileContexts.current[kind]={handle:fileContexts.current[kind]?.handle??null,metadata,source:text,title,options:{...exportOptions,paper:page as ExportOptions['paper'],orientation:orientation as ExportOptions['orientation']}};
  activate(k);setKind(k);const prior=fileContexts.current[k];setText(prior?.source??SAMPLES[k].text);setTitle(prior?.title??SAMPLES[k].title);const options=prior?.options??DEFAULT_EXPORT;setExportOptions(options);setPage(options.paper);setOrientation(options.orientation);setInputDirty(false);
 }
 ...
 async function save(saveAs=false){
  ...
  try{const result=await host.saveProject(project,fileContexts.current[kind]?.handle??null,saveAs);
 ...
 function reopen(){if(guardDraft())return;if(!busy)setModal('files')}
 async function openProject(){
  setModal(null);if(busy)return;
  try{const file=await host.openProject();if(!file)return;const {project,migrated}=parseProject(file.text);
   if(project.diagram_type!==kind&&dirtyKinds[project.diagram_type]&&!window.confirm('将替换另一图种尚未保存的项目，是否继续？'))return;
   const options=readExport(project.metadata.export_options);load(project,migrated);setKind(project.diagram_type);setText(project.source_text);const selected=project.versions.find(v=>v.revision===project.current_revision)!;setTitle(typeof project.metadata.draft_title==='string'?project.metadata.draft_title:selected.spec.title);setDirection(project.metadata.draft_direction==='RIGHT'?'RIGHT':selected.spec.diagram_type==='flowchart'?selected.spec.direction:'DOWN');setBidMode(typeof project.metadata.bid_mode==='string'?project.metadata.bid_mode:'unknown');setExportOptions(options);setPage(options.paper);setOrientation(options.orientation);setInputDirty(false);
   fileContexts.current[project.diagram_type]={handle:migrated?null:file.handle,metadata:project.metadata,source:project.source_text,title:selected.spec.title,options};setRunEvents([]);setAttempts({});setRunStatus('idle');
   notify(migrated?'旧版 0.9 项目已显式迁移，请另存为 1.0；原文件未覆盖。':'项目已打开，已恢复布局和全部版本。');
  }catch(e){if((e as Error).name!=='AbortError')notify((e as Error).message)}
 }
 function requestOpen(){if(dirty||inputDirty)setModal('open-confirm');else void openProject()}
 function changeExport(patch:Partial<ExportOptions>){setExportOptions(o=>({...o,...patch}));if(patch.paper)setPage(patch.paper);if(patch.orientation)setOrientation(patch.orientation);changeInput(true)}
 async function exportFile(format:'png'|'svg'){if(!current)return;try{host.exportImage(await imageBlob(current,{...exportOptions,paper:page as ExportOptions['paper'],orientation:orientation as ExportOptions['orientation']},format),`diagram-v${current.revision}.${format}`);notify(`${format.toUpperCase()} 已生成，请检查浏览器下载。`)}catch(e){notify((e as Error).message)}}
 ...
 return <div className="app-shell">
  ...
  <dialog ref={dialogRef} onCancel={()=>{setModal(null);setProposal(null)}}><button className="dialog-close icon-button" aria-label="关闭弹窗" onClick={()=>{setModal(null);setProposal(null)}}><X size={20}/></button>{modal==='settings'?<ModelSettings onSaved={()=>{setModal(null);setProvider('已连接模型');notify('模型配置已保存。')}}/>:modal==='files'?<><h2>本地项目</h2><p className="muted">一个 JSON 文件保存当前图表及全部版本。保存默认覆盖原文件，另存为选择新位置。</p><div className="dialog-actions"><button className="button" onClick={requestOpen}>打开项目</button><button className="button" disabled={!current||busy} onClick={()=>{setModal(null);void save(true)}}>另存为</button><button className="button primary" disabled={!current||busy} onClick={()=>{setModal(null);void save()}}>保存项目</button></div></>:modal==='open-confirm'?<><h2>有未保存修改</h2><p className="muted">打开其他项目将替换当前图表。请先保存，或明确放弃当前未保存修改。</p><div className="dialog-actions"><button className="button" onClick={()=>setModal(null)}>继续编辑</button><button className="button primary" onClick={()=>openProject()}>放弃修改并打开</button></div></>:modal==='export'?<><h2>图表输出预览</h2><p className="muted">v{current?.revision} · 使用当前快照中的图形与布局</p><div className="export-options"><label>纸型<select aria-label="导出纸型" value={page} onChange={e=>changeExport({paper:e.target.value as ExportOptions['paper']})}><option>A4</option><option>A3</option></select></label><label>方向<select aria-label="导出方向" value={orientation} onChange={e=>changeExport({orientation:e.target.value as ExportOptions['orientation']})}><option value="portrait">竖向</option><option value="landscape">横向</option></select></label><label>图题<select aria-label="导出图题" value={String(exportOptions.includeTitle)} onChange={e=>changeExport({includeTitle:e.target.value==='true'})}><option value="true">包含图题</option><option value="false">不含图题</option></select></label><label>背景<select aria-label="导出背景" value={exportOptions.background} onChange={e=>changeExport({background:e.target.value as ExportOptions['background']})}><option value="white">白色</option><option value="transparent">透明</option><option value="custom">自定义</option></select></label>{exportOptions.background==='custom'&&<label>背景颜色<input aria-label="导出背景颜色" type="color" value={exportOptions.color} onChange={e=>changeExport({color:e.target.value})}/></label>}</div><div className="export-image">{current&&<ExportSheet snapshot={current} options={{...exportOptions,paper:page as ExportOptions['paper'],orientation:orientation as ExportOptions['orientation']}}/>}</div><p className="scope-note">300 DPI · 长图不分页 · AI 补充不阻止导出</p><div className="dialog-actions"><button className="button" onClick={()=>setModal(null)}>返回编辑</button><button className="button" onClick={()=>exportFile('svg')}>下载 SVG</button><button className="button primary" onClick={()=>exportFile('png')}><ArrowDownToLine size={16}/>下载草稿 PNG</button></div></>:modal==='history'?<VersionHistory history={history} currentRevision={current?.revision??null} onSelect={switchVersion} onRestore={()=>{current&&append({...current.version,origin:'restore'});setModal(null);notify('已恢复所查看版本内容，保存为新版本。');}}/>:modal==='replace'?<><h2>重新生成草稿？</h2><p className="muted">将使用左侧内容、图题和版式创建新版本。当前预览中的修改不会带入新草稿，历史版本保留。</p><div className="dialog-actions"><button className="button" onClick={()=>setModal(null)}>继续编辑</button><button className="button primary" onClick={()=>generate(true)}>确认重新生成</button></div></>:<><div className="dialog-symbol"><Sparkles size={24}/></div><h2>体验一张图表的完整流程</h2><ol className="help-steps"><li>选择流程图或甘特图，载入内置示例。</li><li>生成草稿，从右侧缩略图选择样式模板。</li><li>点击小锁解锁后，可连续修改文字和拖动节点；完成后点击确认，再应用本次修改。</li><li>在版本记录中查看和恢复完整快照。</li></ol><p className="scope-note">在模型设置中配置接口后即可生成。项目须手动保存；本地项目中可打开或另存为，下载弹窗支持 PNG 和 SVG。</p><button className="button primary full" onClick={()=>setModal(null)}>开始体验 <ArrowRight size={16}/></button></>}</dialog>
 </div>
}
