...
import {ArrowDownToLine,ArrowRight,Check,CircleHelp,ClipboardList,FileText,GitBranch,History,LayoutGrid,LoaderCircle,LockKeyhole,Maximize2,Minus,Plus,Save,Settings2,Sparkles,UnlockKeyhole,X,RotateCcw} from 'lucide-react';
...
const TaskWorkbench=memo(function TaskWorkbench({task,active,navigation,host,onReport,onRegister}:{task:WorkspaceTask;active:boolean;navigation:ReactNode;host:HostAdapter;onReport:(id:string,summary:TaskSummary)=>void;onRegister:(id:string,control:TaskControls)=>(()=>void)}){
 ...
 const [toast,setToast]=useState(''),[busy,setBusy]=useState(false),[proposal,setProposal]=useState<Proposal|null>(null),[modal,setModal]=useState<'preview'|'export'|'help'|'replace'|'history'|'settings'|null>(null),[zoom,setZoom]=useState(100),[inputDirty,setInputDirty]=useState(false),[locked,setLocked]=useState(true);
 ...
 useEffect(()=>{getHealth().then(h=>setProvider(h.provider==='stub'?'替身模型':'已连接模型')).catch(()=>setProvider('后端未连接'));return()=>{void batchRef.current?.cancel().catch(()=>{});controller.current?.abort();if(jobRef.current)void cancelGeneration(jobRef.current).catch(()=>{})}},[]);
 ...
 function sample(k:Kind){if(k===kind||busy||guardDraft())return;
  ...
  clearCandidates();activate(k);setKind(k);if(TEMPLATES.find(t=>t.id===templateId)?.kind&&TEMPLATES.find(t=>t.id===templateId)?.kind!==k)setTemplateId('classic');const prior=fileContexts.current[k];setText(prior?.source??(task.number===1?SAMPLES[k].text:''));setTitle(prior?.title??'');const options=prior?.options??DEFAULT_EXPORT;setExportOptions(options);setInputDirty(false);
 }
 function loadSample(){if(busy)return;setText(SAMPLES[kind].text);setTitle('');changeInput(true)}
 function switchVersion(snapshot:Snapshot){if(guardDraft())return;select(snapshot.revision);setShowCandidates(false);setModal(null);notify(`已切换到 v${snapshot.revision}，可查看或继续编辑。`)}
 async function generateCandidates(){
  ...
  const batch=startCandidateBatch({source_text:text,direction,title,style},setCandidates,layoutFlow);batchRef.current=batch;
  try{
 ...
 const stage=(current||comparing)?(modal==='export'?2:1):0;
 return <div className="app-shell task-workbench" hidden={!active} data-task-id={task.id}>
  ...
  <div className="workspace"><header><div className="header-title"><strong>标绘<span> / </span></strong><span className="active-task-title" title={task.name||title}>{task.name||title.trim()||`任务 ${task.number}`}</span><span className="demo-badge">本地工作台</span></div><div className="header-actions"><span className="save-status"><i/>{comparing?'候选待选择':pending?'有待确认修改':dirty||inputDirty?'有未保存修改':current?'已保存':'本地工作空间'}</span><button className="button" disabled={!current||busy||comparing} onClick={()=>save()}><Save size={16}/>保存项目</button><button className="button primary" disabled={!current||busy||comparing} onClick={exportPreview}><ArrowDownToLine size={16}/>下载草稿 PNG</button></div></header>
  <div className="project-heading"><div><div className="eyebrow">DIAGRAM STUDIO</div><h1>把方案，变成清晰的图表。</h1></div><div className="local-pill"><span/>{provider} · 本机服务</div></div>
  <nav className="steps" aria-label="制作流程">{['输入内容','预览与修改','导出'].map((label,i)=><div className={i===stage?'step selected':i<stage?'step done':'step'} key={label} aria-current={i===stage?'step':undefined}>{i===2?<button className="step-action" disabled={!current||busy||comparing} onClick={exportPreview}><span className="step-number">03</span><span>{label}</span></button>:<><span className="step-number">{i<stage?<Check size={14}/>:String(i+1).padStart(2,'0')}</span><span>{label}</span><span className="step-line"/></>}</div>)}</nav>
  <main className="editor-grid"><section ref={stylePanelRef} className="input-panel panel"><div className="panel-tabs"><button className={panel==='input'?'selected':''} onClick={()=>setPanel('input')}><FileText size={16}/>内容输入</button><button className={panel==='style'?'selected':''} onClick={()=>setPanel('style')}><Settings2 size={16}/>版式设置</button></div>
   ...
   <label className="field-label" htmlFor={`${task.id}-bid-mode`}>技术标类型 <span className="required">*</span></label><select id={`${task.id}-bid-mode`} disabled={busy} value={bidMode} onChange={e=>{setBidMode(e.target.value);changeInput(true)}}><option value="unknown">尚不确定</option><option value="open">明标</option><option value="blind">暗标</option></select>
