...
export function App({host=standaloneAdapter}:{host?:HostAdapter}){
 ...
 useEffect(()=>{const handler=(e:BeforeUnloadEvent)=>{if(Object.values(dirtyKinds).some(Boolean)||inputDirty){e.preventDefault();e.returnValue=''}};window.addEventListener('beforeunload',handler);return()=>window.removeEventListener('beforeunload',handler)},[dirtyKinds,inputDirty]);
 ...
 function selectTemplate(id:string){if(busy)return;if(current){const p=templateProposal(current,id);if(p.before!==p.after)setProposal(p)}else{setTemplateId(id);changeInput(true)}}
 function sample(k:Kind){if(busy)return;
 ...
 async function save(saveAs=false){
  const project=latestProject.current;if(!project||busy)return;const serialized=JSON.stringify(project);setBusy(true);
 ...
 function reopen(){if(!busy)setModal('files')}
 ...
 function exportPreview(){if(current&&!busy)setModal('export')}
 ...
 return <div className="app-shell">
  ...
  <div className="workspace"><header><div className="header-title"><strong>标绘<span> / </span></strong><span>标书图表工作台</span><span className="demo-badge">本地工作台</span></div><div className="header-actions"><span className="save-status"><i/>{dirty||inputDirty?'有未保存修改':current?'已保存':'本地工作空间'}</span><button className="button" disabled={!current||busy} onClick={()=>save()}><Save size={16}/>保存项目</button><button className="button primary" disabled={!current||busy} onClick={exportPreview}><ArrowDownToLine size={16}/>下载草稿 PNG</button></div></header>
