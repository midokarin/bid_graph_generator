...
export function RunStream({ status, events, expanded, onExpandedChange, attempts }: Props) {
  ...
  return <section className={`run-stream ${expanded ? 'expanded' : ''} ${status}`} aria-label="运行详情">
    ...
    {expanded && <div className="run-stream-body" id="run-stream-log">
      <div className="run-stream-heading"><span><TerminalSquare size={14}/>实时运行日志</span><span>JSON 流 · 校验完成后渲染</span></div>
      ...
      {Object.entries(attempts).map(([attempt,text])=><details key={attempt}><summary>{attempt==='0'?'首次生成 JSON':`第 ${attempt} 次修复 JSON`}</summary><pre className="json-stream">{text}</pre></details>)}
    </div>}
  </section>;
}
