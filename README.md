...
当前交付为 **P0 会话二：双图渲染、工作台编辑与版本链路**。
会话二交付与视觉对照见 [交付记录](docs/session-two-delivery.md)。
...
保存项目和下载按钮保留原入口，但本会话不写项目文件、不下载 PNG/SVG；页面关闭会丢失内存版本。
...
项目格式和图表 Schema 版本分别为 `1.0`。业务 `spec`、`layout`、`style` 分开；版本保存完整快照、补充项和说明。项目 `metadata` 是唯一自由扩展区。迁移入口目前仅接收 `1.0`，旧格式及未知未来格式明确拒绝；Demo 数据不隐式迁移。版本以完整契约快照保存在内存中；编辑从当前查看版本创建新版本，恢复也分配新版本号。两图种各自保留版本链，不裁剪旧版本。文件 UI 尚未实现。
...
下一会话须重验质量命令和当前交付，再实现项目文件、正式导出、设置与真实模型闭环。当前没有真实模型验收，不代表 P0 总验收完成。
